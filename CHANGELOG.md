...
### Added
- **Case Study Filters**: Add Project Rescue filter category with keywords for rescue, remediation, gap analysis, vendor handoff content
...
### Fixed
- **Internal Link Checker**: Only accept 2xx status codes as valid responses
- **Internal Link Checker**: Filter xmlrpc.php and redirect false positives from error reporting
- **External Link Checker**: Accept 2xx, 3xx, and 403 status codes as valid responses
- **External Link Checker**: Add error filtering for xmlrpc.php and redirect false positives
- **Link Checker Issues**: Only create GitHub issues when real broken links exist after filtering

## [2.50.2] - 2026-02-14

### Fixed
- **Link Checkers**: Rewritten to crawl the live 84em.com site via XML sitemap instead of scanning repo source files, ensuring all published pages and dynamically generated links are checked
- **Internal Link Checker**: Fetches sitemap, crawls every page, and validates all links pointing to 84em.com
- **External Link Checker**: Fetches sitemap, crawls every page, and validates all links pointing to third-party domains
...
### Added
- **Ghost Inspector Badge** - Added Ghost Inspector status badge to README.md for test suite visibility

## [2.49.2] - 2026-01-26

### Changed
- **Hero Lazy Load** - Updated fallback image to 2025 desktop background (`includes/hero-lazy-load.php`)
- **CLI Hero Update** - Updated hero image URL to 2025 desktop background (`includes/cli-hero-update.php`)
...
### Changed
- **Hero CSS Variables** - All hero styling now uses CSS variables for maintainability (`assets/css/utilities.css`, `includes/cli-hero-update.php`)
  - `--hero-min-height: 250px` - Hero container minimum height
  - `--hero-overlay-bg: #1e1e1e8c` - Overlay background color
  - `--hero-content-width: 1280px` - Content container width
  - `--hero-bg-position: 60% 45%` - Background image position
  - `--hero-text-color: #ffffff` - Hero text color
  - `--hero-btn-radius-small: 0px` - Button corner radius (small)
  - `--hero-btn-radius-large: 30px` - Button corner radius (large)
  - All 24 pages updated including homepage
  - Added `--include-homepage` flag to WP-CLI command
...
### Fixed
- **Deploy Script** - Fixed heredoc syntax error in `deploy-ai-integration-page.sh`
  - Split content into separate HTML file (`deploy-ai-integration-content.html`)
  - Script now reads content from external file to avoid shell escaping issues
...
### Changed
- **Security Review Workflow** - Add manual trigger support (`workflow_dispatch`)
...
### Added
- **PHPCS Workflow** - WordPress Coding Standards validation via GitHub Actions (`phpcs.yml`)
  - Uses `wp-coding-standards/wpcs` package
  - Configured for modern PHP 8 features (short arrays, named parameters)
  - Non-blocking: reports violations as annotations without failing builds
- **PHPCS Configuration** - Custom ruleset for theme (`phpcs.xml`)
...
### Added
- **Dynamic Years Shortcodes** - Auto-updating experience year calculations (`includes/dynamic-years.php`)
  - `[dev_years]` - Years since 1995 (programming experience)
  - `[wp_years]` - Years since 2012 (WordPress experience)
  - `[years_since year="XXXX"]` - Custom year calculation
  - Replaces hardcoded "30 years" and "13 years" references across 62 content items

- **Dynamic Years Migration CLI** - WP-CLI commands for content migration (`includes/cli-dynamic-years.php`)
  - Command: `wp 84em dynamic-years test` - Test shortcode functionality
  - Command: `wp 84em dynamic-years stats` - View migration statistics
  - Command: `wp 84em dynamic-years migrate` - Execute migration with `--dry-run` support

## [2.34.1] - 2026-01-07

### Changed
- **Client Logo Gallery** - Removed grayscale filter, logos now display in full color (`assets/css/utilities.css`)
...
- **HTML Separator Block** - Lightweight UAGB separator replacement for block editor (`assets/js/html-separator.js`, `includes/enqueue.php`)
  - Registers `uagb/separator` block type with code icon SVG
  - Matches existing UAGB separator markup for seamless transition
  - Enqueued only in block editor context

### Changed
- **Brand Green Color** - Updated from #3d5c04 to #9ff400 for better visibility (`theme.json`)

- **Case Study Filter Keywords** - Refined keyword matching (`includes/case-study-filters.php`)
  - Expanded security filter with additional keywords
  - Removed "assessment" from healthcare keywords
...
- **Testimonials Page Migration** - Replaced legacy synced pattern testimonials with Easy Testimonial Blocks plugin
  ...
  - Styling handled entirely by Easy Testimonial Blocks plugin
...
### Removed
- **Testimonials CSS** - Deleted `assets/css/testimonials.css` and removed enqueue from `includes/enqueue.php`
